...
def ball_movement():
    ...
    speed = 7
    if start:
        ball_speed_x = speed * random.choice((-3, 3))  # Randomize initial horizontal direction
    ...
    hit_sound = pygame.mixer.Sound("pong_hit2.wav")  # load your sound
    hit_sound.set_volume(1.0)  # ensure it's not muted
    if ball.colliderect(player):
...
player_height = 15
player_width = 200 # Original width = 100
...
while True:
    ...
    # Visuals
    ball_color = pygame.Color('deeppink')
    red = pygame.Color('red')
    screen.fill(bg_color)  # Clear screen with background color
    pygame.draw.rect(screen, ball_color, player)  # Draw player paddle
    # TODO Task 3: Change the Ball Color
    pygame.draw.ellipse(screen, ball_color, ball)  # Draw ball
    player_text = basic_font.render(f'{score}', False, ball_color)  # Render player score
